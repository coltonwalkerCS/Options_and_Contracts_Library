...
def getConvenienceYield_XMoContract(numMonths, forward_price, interest_rate, annual_storage_cost, annual_insurance_cost, cash_price):
...
def simpleStockForwardPrice(stock_price, interest_rate, time_to_maturity, dividend, time_to_next_div):
    ...
    forwardPrice = stock_price * (1 + interest_rate * time_to_maturity) - (numDividends * dividend)
    print(forwardPrice)
    assert(forwardPrice == 69.02)
    return forwardPrice
